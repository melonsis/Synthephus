...
def mwem_pgm(
    ...
    if synp_model_load is not "0":
        est = pickle.load(open(synp_model_load, "rb"))
    cliques = []
    ...
    if cliquesave is not '0':
        ...
        cliquepd.to_csv(cliquesave+"/cliques_temp.csv",index=False) #Synthephus: Save all selected cliques
    if synp_model_save is not '0':
        pickle.dump(est, open(synp_model_save, "wb"))
...
def default_params():
    ...
    params["cliquesave"] = "./data/cliques"
    params["synp_model_load"] = '0'
    params["synp_model_save"] = "./data/model/temp_model.pkl"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--synp_model_load", type=str, help="path to load synthetic model")
    parser.add_argument("--synp_model_save", type=str, help="path to save synthetic model")
    ...
    synth = mwem_pgm(
        ...
        cliquesave=args.cliquesave,
        synp_model_load=args.synp_model_load,
        synp_model_save=args.synp_model_save,
    )
